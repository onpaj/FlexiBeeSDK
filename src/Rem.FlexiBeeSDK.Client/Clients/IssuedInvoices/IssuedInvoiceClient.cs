...
namespace Rem.FlexiBeeSDK.Client.Clients.IssuedInvoices
{
    public class IssuedInvoiceClient : ResourceClient, IIssuedInvoiceClient
    {
        ...
        public async Task<IssuedInvoice> GetAsync(string code, CancellationToken cancellationToken = default)
        {
           ...
           var found = await GetAsync<IssuedInvoice>(query, cancellationToken);
        ...
        public Task<OperationResult<OperationResultDetail>> SaveAsync(IssuedInvoice invoice,
            CancellationToken cancellationToken = default) => PostAsync(CreateSaveRequest(invoice), cancellationToken: cancellationToken);

        private object CreateSaveRequest(IssuedInvoice invoice)
        {
            return new Dictionary<string, object>()
            {
                { ResourceIdentifier, invoice }
            };
        }
    }
}